 /*
Firmware for underwater load cell logger.

Author: Bill DeVoe, Marine Resource Scientist III, Maine Department of Marine Resources
...
3.2: 11/19/2020 - Added option for monitoring LED, indicates logger recording, low battery, and if trip value has been reached.
  Fixed bug where calibration values were not being written to config file.

Description:
Logs values from a SparkFun Qwiic Scale NAU7802 to a SD card and specified intervals. Also allows for calibration of the load cell 
over a serial interface. Intended for use on a Adafruit Feather M0 Express with an Adalogger shield, but should work on other
...
#define DEFAULT_LOG_INTERVAL 250 
...
// The built in LED on pin 13 is available, but was not used in an earlier iteration for the Arduino Nano
// due to pin 13 on that board being the SPI CLK pin.
// Setting pin 17 LOW will turn on the RX led instead, eliminating the need for an off-board LED.
// With the addition of the optional off-board RGB LED, this is mostly unnecessary.
#define ERROR_LED 17 
